...
function mergeImportedRecording(existingRecording: typeof recordings.$inferSelect, importedRecording: Recording) {
  return {
    ...
    ownershipStatus: importedRecording.ownershipStatus ?? existingRecording.ownershipStatus ?? 'owned',
    ownershipNote: importedRecording.ownershipNote ?? existingRecording.ownershipNote ?? undefined,
    ...
    await db.transaction(async (tx) => {
      for (const importedRecording of importedRecordings) {
        ...
        const [newRecording] = await tx.insert(recordings).values({
          ...
          ownershipStatus: importedRecording.ownershipStatus ?? 'owned',
          ownershipNote: importedRecording.ownershipNote ?? null,
